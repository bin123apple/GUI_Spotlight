...
def _normalize_poly(bbox_xywh_or_poly: Sequence) -> List[Tuple[float, float]]:
    if not isinstance(bbox_xywh_or_poly, (list, tuple)) or len(bbox_xywh_or_poly) < 4:
        raise ValueError("bbox_xywh_or_poly shape must be a list/tuple with at least 4 elements")
    ...
    if all(isinstance(v, (int, float)) for v in arr):
        if len(arr) % 2 != 0:
            raise ValueError("Must have an even number of coordinates for point pairs")
        it = iter(arr)
...
def draw_viz(
    ...
    try:
        ...
        print(f"[draw_viz] cannot normalize bbox: {bbox_xywh}. Error: {e}")
        poly = None
    ...
    if pred not in ("none", None):
        try:
            ...
            print(f"[draw_viz] Failed to draw pred point: {pred}. Error: {e}")
    ...
    for sid, cats in id2cats.items():
        ...
        if len(cats) == 1:
            ...
            if verbose:
                ...
                print(f"[WARN] ID {sid!r} exists in multiple categories {cats}, chose {chosen} over {others} by '{resolve}' rule.")
...
# def _update_cat_stats(cat_stats: dict, cat_key: Optional[str], hit: bool, is_none: bool):
#     if not cat_key or cat_key not in cat_stats:
#         return
#     cat_stats[cat_key]["total"] += 1
#     if hit:
#         cat_stats[cat_key]["hits"] += 1
#     if is_none:
#         cat_stats[cat_key]["none"] += 1
...
def evaluate_dataset_in_batches(
    ...
    for i, item in enumerate(data):
        try:
            ...
            prompt = item["prompt_to_evaluate"]
            bbox_xywh = item["bbox"]
            ...
            total += 1
            misses += 1
            print(f"[WARN] item {i} ({item.get('id')}) failed to load: {e}")
    if not entries:
        print("There is no valid entry to process.")
        return
...
def build_argparser():
    ...
    parser.add_argument("--output_dir", type=str, default="viz_out", help="Visualization output directory")

    # 批量评测
    parser.add_argument("--dataset_json", type=str, help="Dataset JSON file path")
    parser.add_argument("--images_dir", type=str, help="Directory containing images (if not in dataset_json path)")
...
def main():
    ...
    if args.dataset_json:
        ...
        if not args.prompt or not args.image_path:
            raise SystemExit("For single sample evaluation, both --prompt and --image_path must be provided.")
